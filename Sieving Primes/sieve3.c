...
int main (int argc, char *argv[])
{
   ...
   unsigned long long int  block_n = 262143; /* try from half of 2048KB (n=262143) */
   ...
   /* Add you code here  */
    ...
    do {
        ...
        for (i = block_first; i < block_n; i += prime) marked[i] = 1;

            while (local_prime_marked[++index]);
            prime = index + 2;
        
        block_low = block_high + 2;
        if(block_high >= high_value ) block_high = high_value;
		else block_high = block_low + 2*(block_n - 1);
    }
    } while (block_low <= high_value && prime * prime <= n);
